...
//my values
unsigned long play_time = 300000; // plays 5 min
unsigned long wayt_time = 3600000; // wayts 1h
unsigned long start_old = 0; // place where previous start is saved
...
void loop() {
  ...
  if ((start_old + wayt_time) < millis() || start_old == 0) {
    unsigned long start = millis();
    unsigned long end = start + play_time;
